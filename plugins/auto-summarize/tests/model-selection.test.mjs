...
test("selects the configured auto-summarize model before hardcoded candidates", async () => {
  const { selectCheapModel } = await loadSelectionModule();
  const models = [
    model("amazon-bedrock", "us.anthropic.claude-haiku-4-5"),
    model("openai-codex", "gpt-5.4-mini"),
  ];

  const selected = selectCheapModel(models, allAuthed, {
...
test("uses the scout subagent model as the existing cheap-tier fallback", async () => {
  const { selectCheapModel } = await loadSelectionModule();
  const models = [
    model("amazon-bedrock", "us.anthropic.claude-haiku-4-5"),
    model("openai-codex", "gpt-5.4-mini"),
...
test("does not let legacy autoSummary.model override the scout cheap-tier setting", async () => {
  const { selectCheapModel } = await loadSelectionModule();
  const models = [
    model("amazon-bedrock", "us.anthropic.claude-haiku-4-5-20251001-v1:0"),
    model("openai-codex", "gpt-5.4-mini"),
...
test("falls back to known cheap candidate IDs when settings do not specify a model", async () => {
  const { selectCheapModel } = await loadSelectionModule();
  const models = [
