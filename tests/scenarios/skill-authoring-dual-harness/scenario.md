...
- [ ] The skill uses the documented cross-harness dynamic context pattern: `` !`command` `` inline syntax with the fallback instruction (see platform-guide.md §1 "! Backtick Fallback").
...
| uses ! backtick + fallback | Pass | Used the inline `` !`command` `` syntax and included the fallback instruction "If the fields below show commands rather than output, run each one first." |
