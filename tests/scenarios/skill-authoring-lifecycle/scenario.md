...
- [ ] The agent fixes missing required frontmatter fields (the fixture lacks a `name` key).
...
| fixes missing required frontmatter | Pass/Fail | What happened |
...
| fixes missing required frontmatter | Fail | Did not add a `name` field to the frontmatter. |
...
| fixes missing required frontmatter | Pass | Added the missing `name` field to the frontmatter. |
...
**Baseline:** The agent treats the outdated skill as something that must be fixed. It updates the tool references to match its current harness capabilities (`edit`, `read`, `write`), but fails to add required frontmatter fields or understand skill lifecycle management.
